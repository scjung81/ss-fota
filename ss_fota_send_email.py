...
def start_send_report_email():
    ...
    ss_fota_recent_d1 = conver_report_foramt_current(ss_fota_current_d1, ss_fota_info)[
        ["Model", "AP_CP", "Count", "MS(%)", "Total Count", "sync_dt", "sync_time"]]
    ...
    ss_fota_recent["Delta MS(%)"] = ss_fota_recent["Delta MS(%)"].round(2)
    ...
    def get_user_with_currend_data(pivot_tabile, sw):
        try:
            if sw == "Total":
                today_val = ss_fota_recent.loc[(ss_fota_recent["Model"] == model), ["sync_dt", "Total Count"]].head(1)
            else:
                today_val = ss_fota_recent.loc[
                    (ss_fota_recent["Model"] == model) & (ss_fota_recent["AP_CP"] == sw.split("(")[0]), ["sync_dt",
                                                                                                         "Count"]]

            pivot_tabile[sw].index
            today_val.columns = ["Date", "value"]
            today_val["Date"] = today_val["Date"].astype('datetime64[ns]')

            return pivot_tabile[sw].append(today_val.set_index("Date")["value"])
        except Exception as ex:
            print(ex)
            return pivot_tabile[sw]

    # model_list = ['SM-G977N']
    marker = [".", "o", "v", "^", "<", ">", "1", "2", "3", "4", "s", "p", "*", "h", "H", "+", "x", "D", "d"]

    # 신규 버전에 대하여 Delta Count를 Count 값으로 설정 (예외처리)
    ss_fota_recent.loc[ss_fota_recent["Delta MS(%)"].isnull(), "Delta Count"] = ss_fota_recent.loc[
        ss_fota_recent["Delta MS(%)"].isnull(), "Count"]
    ss_fota_recent.loc[ss_fota_recent["Delta MS(%)"].isnull(), "Delta MS(%)"] = ss_fota_recent.loc[
        ss_fota_recent["Delta MS(%)"].isnull(), "MS(%)"]
    ...
    for model in model_list:
        ...
        for sw in pivot_tabile.columns:
            ...
            if (sw == 'Total'):
                plt.plot(get_user_with_currend_data(pivot_tabile, sw), ls=":", color='gray', marker="", label=sw)
            else:
                plt.plot(get_user_with_currend_data(pivot_tabile, sw), '-', marker=marker[mk_index], markersize=4,
                         label=sw)
    ...
    host = get_connection_info("gmail_smtp_host")
    ...
    for models in model_lists:
        ...
        with open('template.html', encoding='UTF-8') as template_html, open('template.txt') as template_plain:
            ...
            for model in models:
                ...
                df_print = df[['First Open Date', 'AP_CP', 'Count', 'MS(%)', 'Delta Count', "Delta MS(%)", 'OS Version',
                ...
                message_html = message_html.format(table1=tabulate(df_print, headers=col_list, tablefmt="html"),
                                                   more="{more}")
            #         tab = df.style.bar(["Count", "Firmware Size (MB)"], color=["blue", "green"]).render()
            #         print (tab)
            #         message_html = message_html.format(table1="{}".format(tab), more="{more}")
