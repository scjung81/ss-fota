...
def StartMaeDb_last90days():

    #마지막 폴더 찾기
    filepath = "data"
    # lastdate = max([filepath +"/"+ f for f in os.listdir(filepath)], key=os.path.getctime)
    lastdate = max([filepath +"/"+ f for f in os.listdir(filepath) if re.match(r'[^.]+', f)], key=os.path.getctime) #. 으로 시작하는 파일명 제외
    ...
    ss_fota = pd.read_excel(os.path.join(os.getcwd(),lastdate,f"ss_fota_device_by_version_{dt}_{time}.xlsx"),header = 3, na_values = 'NaN')
    ss_fota['sync_dt'] = dt
    ss_fota['sync_time'] = time

    ss_fota.rename(columns={"Device Count / Total": "Device Count"}, inplace=True)
    ...
    #날짜/모델별 Total count 추가
    tot_device_count = ss_fota["Device Count"].groupby([ss_fota["Date"], ss_fota["Model"]]).sum().reset_index()
    tot_device_count.rename(columns = {'Device Count' : "Total Device Count_with_OTHERS"}, inplace = True)

    tot_device_count
    merged = pd.merge(ss_fota, tot_device_count)
    merged["MS_with_OTHERS"] = merged["Device Count"]/merged["Total Device Count_with_OTHERS"]*100
    ss_fota = merged
    ...
    a1 = pd.merge(ss_fota_data, plm_sw, left_on=['Model', 'ap_cp'], right_on=['model', 'ua_ver'], how='inner' )  #PLM 에 없는 Data 는 제거
    a2 = pd.merge(ss_fota_data, plm_sw, left_on=['Model', 'ap'], right_on=['model', 'ua_ver'], how='inner' )  #PLM 에 없는 Data 는 제거


    # In[15]:


    ss_fota_final = pd.concat([a1, a2]).sort_values(by=["Model", "ap_cp"])
    ss_fota_final
    ...
    ss_fota=ss_fota_final

    #날짜/모델별 Total count 추가
    tot_device_count = ss_fota["Device Count"].groupby([ss_fota["Date"], ss_fota["Model"]]).sum().reset_index()
    tot_device_count.rename(columns = {'Device Count' : "Total Device Count"}, inplace = True)

    tot_device_count
    merged = pd.merge(ss_fota_final, tot_device_count)
    merged["MS"] = merged["Device Count"]/merged["Total Device Count"]*100
    ...
    lstday = lastdate.split("/")[1].split("_")[0]
    fname  = lastdate+"/ss_fota_last90days_" + lstday + ".csv"
    fname2  = lastdate+"/ss_fota_last90days.csv"
    print(fname, fname2)
