...
def start_send_report_email_custom(model_lists, email_list):
    ...
    ss_fota_recent = pd.merge(ss_fota_recent, ss_fota_recent_d1, on=['Model', 'AP_CP'], how='left')  # #Merge 방법을 left 로 변경 : 최든 자료를 기준으로 Data 취합, (전일자 Data에 잘못된 버전이 들어와 있을 경우 계속적으로 남아있게 되는 문제 수정)
